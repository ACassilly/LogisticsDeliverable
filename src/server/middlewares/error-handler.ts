...
function handleMongooseValidationError(error: mongoose.Error.ValidationError): ErrorResponse {
  ...
  return { success: false, error: 'Validation Error', message: 'Invalid data provided', statusCode: 400, details: errors };
}

function handleMongooseDuplicateKeyError(error: { keyValue: Record<string, unknown> }): ErrorResponse {
  const field = Object.keys(error.keyValue)[0];
  const value = error.keyValue[field];
  return { success: false, error: 'Duplicate Error', message: `${field} '${value}' already exists`, statusCode: 409 };
}

function handleMongooseCastError(error: mongoose.Error.CastError): ErrorResponse {
  return { success: false, error: 'Invalid ID', message: `Invalid ${error.path}: ${error.value}`, statusCode: 400 };
}

function handleZodError(error: ZodError): ErrorResponse {
  const errors = error.issues.map((err) => ({ field: err.path.join('.'), message: err.message }));
  return { success: false, error: 'Validation Error', message: 'Invalid data provided', statusCode: 400, details: errors };
}
...
export function handleApiError(error: unknown): NextResponse {
  ...
  if (error instanceof ApiError) {
    errorResponse = { success: false, error: error.name, message: error.message, statusCode: error.statusCode };
    ...
    errorResponse = { success: false, error: 'Internal Server Error', message: error.message || 'An unexpected error occurred', statusCode: 500 };
  } else {
    errorResponse = { success: false, error: 'Internal Server Error', message: 'An unexpected error occurred', statusCode: 500 };
  }
...
export function asyncHandler<T extends (...args: unknown[]) => Promise<NextResponse>>(handler: T): T {
