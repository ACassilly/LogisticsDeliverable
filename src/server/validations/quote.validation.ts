...
const itemSchema = z.object({
  ...
  pieceCount: z.number({ message: 'Piece count is required' }).int().min(1),
  palletCount: z.number({ message: 'Pallet count is required' }).int().min(1),
  weight: z.number({ message: 'Weight is required' }).min(1).max(20000),
...
export function transformToGTZShipRequest(data: QuoteRateRequestInput, customerId: string) {
  ...
  return {
    ...
    Items: data.items.map((item) => ({
      PieceCount: item.pieceCount, PalletCount: item.palletCount,
      Length: item.length ?? 0, Width: item.width ?? 0, Height: item.height ?? 0,
      Weight: item.weight, WeightType: item.weightType, ProductClass: item.productClass,
      ...(item.nmfcNumber ? { NmfcNumber: item.nmfcNumber } : {}),
      Description: item.description, PackageType: item.packageType,
      Hazmat: item.hazmat, Stackable: item.stackable,
    })),
