...
export interface CloudinaryUploadResult { url: string; publicId: string; width: number; height: number; format: string; size: number; }
...
export async function uploadImage(file: File, folder = 'blog-images'): Promise<CloudinaryUploadResult> {
  ...
  const result = await new Promise<UploadApiResponse>((resolve, reject) => {
    cloudinary.uploader.upload_stream({ resource_type: 'image', folder, transformation: [{ width: 1200, height: 630, crop: 'limit', quality: 'auto:good', fetch_format: 'auto' }] }, (error, result) => {
      if (error) reject(error); else if (result) resolve(result); else reject(new Error('Upload failed'));
    }).end(buffer);
  });
...
export function extractPublicId(url: string): string | null {
  try { const match = url.match(/\/v\d+\/(.+)\.[a-z]+$/); return match ? match[1] : null; } catch { return null; }
}
