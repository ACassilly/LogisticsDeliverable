...
export async function createCheckoutSession(params: CreateCheckoutParams): Promise<{ sessionId: string; sessionUrl: string }> {
  ...
  const lineItems: Stripe.Checkout.SessionCreateParams.LineItem[] = params.charges
    .map((charge) => ({ price_data: { currency: 'usd', product_data: { name: charge.name || 'Freight Charge' }, unit_amount: Math.max(0, Math.floor(parseFloat(charge.amount.toString()) * 100)) }, quantity: 1 }))
    .filter((item) => item.price_data.unit_amount > 0)
  const finalLineItems = lineItems.length > 0 ? lineItems : [{ price_data: { currency: 'usd', product_data: { name: `Freight — ${params.carrierName}` }, unit_amount: Math.max(1, Math.floor(parseFloat(params.totalRate.toString()) * 100)) }, quantity: 1 }]
