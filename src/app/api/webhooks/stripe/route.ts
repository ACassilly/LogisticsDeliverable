...
import { constructWebhookEvent, type RivenBillingWebhookEvent } from '@/server/services/riven-billing.service'
...
/**
 * Riven billing webhook handler.
 *
 * DEFECT-09: The raw body + signature are forwarded to the Riven billing
 * webhook-verify endpoint, which returns the parsed event. The event shape
 * mirrors the old Stripe event subset (`event.type` + `event.data.object` with
 * `id`, `payment_status`, `payment_intent`), so the downstream booking +
 * Odoo + email logic is unchanged.
 */
export async function POST(request: NextRequest) {
  try {
    const signature =
      request.headers.get('stripe-signature') ||
      request.headers.get('riven-signature') ||
      ''
    if (!signature) {
    ...
    let event: RivenBillingWebhookEvent
    try {
    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (booking && !booking.odooSaleOrderId) {
          ;(async () => {
            try {
              const saleOrderId = await createOdooSaleOrder({
                email: booking.email,
                ...
                items: booking.items.map((item) => ({
                  ...
                  protectFromFreezing: item.protectFromFreezing,
                })),
                carrierName: booking.carrierName,
                carrierCode: booking.carrierCode,
                quoteId: booking.quoteId,
                totalRate: booking.totalRate,
                charges: booking.charges.map((c) => ({ name: c.name, amount: c.amount })),
              ...
              console.error('[Webhook] Odoo sale order creation failed (non-blocking):', err)
            }
          })()
          sendBookingConfirmationEmail({
            ...
            pickup: {
              ...
              pickupDate: booking.pickup.pickupDate,
            },
            delivery: {
            ...
            console.error('[Webhook] Booking confirmation email failed (non-blocking):', err)
          })
        }
        break
      }
    ...
    console.error('Riven billing webhook error:', error)
